...
from braindecode.models import EEGNetv4, EEGNetv1, ATCNet, EEGConformer, EEGITNet, ShallowFBCSPNet
import torch.nn as nn
...
class CustomEEGNetv1(nn.Module):
    def __init__(self, num_classes, in_chans=24, input_window_samples=192, final_conv_length='auto'):
        super(CustomEEGNetv1, self).__init__()

        # 定义 EEGNetv1 模型，调整 input_window_samples 以匹配您的数据
        self.eegnet = EEGNetv1(
            # in_chans=in_chans,
            n_chans=in_chans,
...
if __name__ == '__main__':
    ...
    model = CustomEEGNetv1(num_classes=10, in_chans=24, input_window_samples=192).to(device)  # EVRNet(num_classes=num_classes).to(device)  # 将模型移动到 GPU
    ...
    for epoch in range(num_epochs):
        ...
        if test_accuracy > best_test_acc:
            ...
            # 立即保存最佳模型权重到文件
            torch.save(best_model_weights, './classification_checkpoint/EEGNetV1/best_model_weights.pth')
    ...
    model = CustomEEGNetv1(num_classes=10, in_chans=24, input_window_samples=192).to(device)  # 将模型移动到 GPU
    # 训练完成后，使用最佳模型权重更新模型
    model.load_state_dict(torch.load('./classification_checkpoint/EEGNetV1/best_model_weights.pth'))
    model.eval()
    ...
    plot_and_save_loss_curves(train_losses, test_losses,
                              filename='classification_checkpoint/EEGNetV1/loss_curves.png')