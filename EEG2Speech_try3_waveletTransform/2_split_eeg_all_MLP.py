...
class EVRNet(nn.Module):
    def __init__(self, num_classes, input_size=24, hidden_size=256, mlp_hidden_sizes=(128, 64)):
        ...
        for size in mlp_hidden_sizes:
            layers.append(nn.Linear(prev_size, size))  # 添加全连接层
            layers.append(nn.ReLU())  # 添加非线性激活函数
            prev_size = size

        # 最后一层的输出维度为 hidden_size
        layers.append(nn.Linear(prev_size, hidden_size))

        self.mlp = nn.Sequential(*layers)  # 构建 MLP 模块
    ...
    def forward(self, x):
        ...
        # 应用多层感知机（MLP）
        out = self.mlp(x.permute(0, 2, 1))  # 直接应用 MLP
        ...
        out = self.fc_layer_2(out)  # 最终的全连接层用于分类或回归任务
...
if __name__ == '__main__':
    ...
    for epoch in range(num_epochs):
        ...
        if test_accuracy > best_test_acc:
            ...
            # 立即保存最佳模型权重到文件
            torch.save(best_model_weights, './classification_checkpoint/MLP/best_model_weights.pth')
    ...
    model = EVRNet(num_classes=num_classes).to(device)  # 将模型移动到 GPU
    # 训练完成后，使用最佳模型权重更新模型
    model.load_state_dict(torch.load('./classification_checkpoint/MLP/best_model_weights.pth'))
    model.eval()
    ...
    # 在训练完成后调用此函数
    plot_and_save_loss_curves(train_losses, test_losses, filename='classification_checkpoint/MLP/loss_curves.png')
