...
class EVRNet(nn.Module):
    def __init__(self, num_classes, input_size=24, d_model=32, seq_len=192, max_len=192):
        ...
        # 添加线性变换层，将输入数据的维度从 input_size 转换为 d_model
    ...
    def forward(self, x):
        # 调整输入形状为 (batch_size, timestamps, channels)
        x = x.squeeze(1)  # 移除多余的维度
        x = x.permute(0, 2, 1)  # 调整维度顺序

        # 将输入数据通过线性变换层，转换为 BERT 的隐藏层大小
        ...
        # 获取 BERT 的最后一层输出
        sequence_output = bert_outputs.last_hidden_state  # 形状为 (batch_size, seq_len, d_model)

        # 对序列输出进行池化操作
        sequence_output = sequence_output.permute(0, 2, 1)  # 调整为 (batch_size, d_model, seq_len)
        pooled_output = self.avg_pooling(sequence_output)  # 自适应平均池化
        pooled_output = pooled_output.squeeze(-1)  # 移除最后一个维度，形状变为 (batch_size, d_model)

        # 通过全连接层输出分类结果
        output = self.fc_layer(pooled_output)
...
if __name__ == '__main__':
    ...
    for epoch in range(num_epochs):
        ...
        if test_accuracy > best_test_acc:
            ...
            # 立即保存最佳模型权重到文件
            torch.save(best_model_weights, './classification_checkpoint/Bert/best_model_weights.pth')
    ...
    model = EVRNet(num_classes=num_classes).to(device)  # 将模型移动到 GPU
    # 训练完成后，使用最佳模型权重更新模型
    model.load_state_dict(torch.load('./classification_checkpoint/Bert/best_model_weights.pth'))
    model.eval()
    ...
    # 在训练完成后调用此函数
    plot_and_save_loss_curves(train_losses, test_losses, filename='classification_checkpoint/Bert/loss_curves.png')
