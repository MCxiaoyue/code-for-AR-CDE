...
# test_indices 是你想要用于测试集的数据索引, 和分类器部分的测试集对应
test_indices = [i for i in range(0, 605, 10)]
...
def evaluate(net, dataloader, save_noise=False):
    ...
    if save_noise:
        torch.save(torch.cat(noises, dim=0), 'diffusion_checkpoint1/best_noise.pt')
...
# 在训练循环外部定义保存路径
best_model_path = 'diffusion_checkpoint1/best_model_weights.pth'
last_model_path = 'diffusion_checkpoint1/last_model_weights.pth'
...
# plt.legend()
# plt.grid(True)
# plt.savefig('./diffusion_checkpoint1/loss_curve.png')
# plt.show()
...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
net = ClassConditionedUnet().to(device)
net.load_state_dict(torch.load('diffusion_checkpoint1/best_model_weights.pth'))
net.eval()
...
from EEG_models1 import EVRNet

model = EVRNet(num_classes=10).to(device)  # 将模型移动到 GPU
# 训练完成后，使用最佳模型权重更新模型
model.load_state_dict(torch.load('./classification_checkpoint/EVRNet_mamba/best_model_weights.pth'))
model.eval()
...
# 测试数据的索引
test_indices = [i for i in range(0, image_data_array.shape[0], 10)]  # 第5, 55, 105, 155个数据（基于0索引）
