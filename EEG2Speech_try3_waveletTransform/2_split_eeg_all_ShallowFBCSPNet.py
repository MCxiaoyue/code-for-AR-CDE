...
class CustomShallowFBCSPNet(nn.Module):
    def __init__(self, num_classes, in_chans=24, input_window_samples=192):
        super(CustomShallowFBCSPNet, self).__init__()

        # 初始化 EEGITNet 模型，根据需要调整参数
        self.atcnet = ShallowFBCSPNet(
            n_chans=in_chans,
...
if __name__ == '__main__':
    ...
    model = CustomShallowFBCSPNet(num_classes=10, in_chans=24).to(device)  # EVRNet(num_classes=num_classes).to(device)  # 将模型移动到 GPU
    ...
    for epoch in range(num_epochs):
        ...
        if test_accuracy > best_test_acc:
            ...
            # 立即保存最佳模型权重到文件
            torch.save(best_model_weights, './classification_checkpoint/ShallowFBCSPNet/best_model_weights.pth')
    ...
    model = CustomShallowFBCSPNet(num_classes=10, in_chans=24).to(device)  # 将模型移动到 GPU
    # 训练完成后，使用最佳模型权重更新模型
    model.load_state_dict(torch.load('./classification_checkpoint/ShallowFBCSPNet/best_model_weights.pth'))
    model.eval()
    ...
    # 在训练完成后调用此函数
    plot_and_save_loss_curves(train_losses, test_losses, filename='classification_checkpoint/ShallowFBCSPNet/loss_curves.png')