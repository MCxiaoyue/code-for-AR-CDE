...
from braindecode.models import EEGNetv4, ATCNet, EEGConformer, EEGITNet, ShallowFBCSPNet, EEGInception
import torch.nn as nn
...
class CustomATCNet(nn.Module):
    def __init__(self, num_classes, in_chans=24, input_window_samples=192):
        super(CustomATCNet, self).__init__()

        # 初始化 ATCNet 模型，根据需要调整参数
        self.atcnet = ATCNet(
            n_channels=in_chans,
    ...
    def forward(self, x):
        # 确保输入张量的维度符合 ATCNet 的期望 [batch_size, channels, time_points]
...
# class CustomATCNet(nn.Module):
#     def __init__(self, num_classes, in_chans=24, input_window_samples=192, hidden_size=32):
#         super(CustomATCNet, self).__init__()
#
#         # 初始化 ATCNet 模型，根据需要调整参数
#         self.atcnet = ATCNet(
...
#         x = self.atcnet(x)
#
#         # 假设 ATCNet 输出的形状为 (batch_size, num_classes)
#         # 在这里添加池化操作
#         x = x.unsqueeze(2)  # 扩展维度到 (batch_size, num_classes, 1)
#         x = self.avg_pooling(x)  # 应用池化，结果形状仍为 (batch_size, num_classes, 1)
#         x = x.squeeze(2)  # 移除最后一个维度，恢复为 (batch_size, num_classes)
#
#         x = self.fc_layer_2(x)
#
#         return x
...
if __name__ == '__main__':
    ...
    # 转换为numpy数组
    image_data_array = np.array(image_data) # .transpose(0, 2, 1)
    ...
    model = CustomATCNet(num_classes=10, in_chans=24).to(device)  # EVRNet(num_classes=num_classes).to(device)  # 将模型移动到 GPU
    ...
    for epoch in range(num_epochs):
        ...
        if test_accuracy > best_test_acc:
            ...
            # 立即保存最佳模型权重到文件
            torch.save(best_model_weights, './classification_checkpoint/ATCNet/best_model_weights.pth')
    ...
    model = CustomATCNet(num_classes=10, in_chans=24).to(device)  # 将模型移动到 GPU
    # 训练完成后，使用最佳模型权重更新模型
    model.load_state_dict(torch.load('./classification_checkpoint/ATCNet/best_model_weights.pth'))
    model.eval()
    ...
    # 在训练完成后调用此函数
    plot_and_save_loss_curves(train_losses, test_losses, filename='classification_checkpoint/ATCNet/loss_curves.png')