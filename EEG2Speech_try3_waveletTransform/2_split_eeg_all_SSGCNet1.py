...
from SSGCNet_pak.optimizer import PruneAdam
from SSGCNet_pak.utils import regularized_nll_loss, admm_loss, \
    initialize_Z_and_U, update_X, update_Z, update_Z_l1, update_U, \
    print_convergence, print_prune, apply_prune, apply_l1_prune
from SSGCNet_pak import method
import time
...
class SSGCNnet(nn.Module):
    def __init__(self, channels, num_classes):
        super(SSGCNnet, self).__init__()
        self.channels = channels
        self.aggregate_weightT = nn.Parameter(torch.ones(1, 64, channels))  # 可学习的通道权重矩阵
    ...
    def forward(self, x):
        # print(x.shape)
        x = x.squeeze(1)
        # x 的形状为 (batch_size, channels, timestamps)
        # print(x.shape)
        # # 对通道进行加权聚合
        x = torch.matmul(self.aggregate_weightT, x)  # 输出形状为 (batch_size, 1, timestamps)
        # print(x.shape)
        # 提取特征
        x = self.features(x)  # 输出形状为 (batch_size, 32, timestamps_after_pooling)
        # print(x.shape)
        # # 展平特征图
        # x = x.view(x.size(0), -1)  # 展平为 (batch_size, 32 * (timestamps_after_pooling))

        # 分类
        x = self.classifier(x.view(x.size(0), -1))  # 输出形状为 (batch_size, num_classes)
        # print(x.shape)
...
if __name__ == '__main__':
    ...
    reshaped_data = []
    # 遍历每个样本
    for sample in image_data_array:
        # 将 24x192 展平为一维数组
        flat_sample = sample.flatten()

        # 将展平后的数组按每 512 个数据点分割
        split_parts = np.array_split(flat_sample, 9)

        # 确保每部分恰好有 512 个数据点
        for part in split_parts:
            if len(part) == 512:  # 验证长度是否正确
                reshaped_data.append(part)

    # 将列表转换为 NumPy 数组并调整形状为 (-1, 512)
    image_data_array = np.array(reshaped_data).reshape(-1, 512)

    methods = {'WNFG2': method.overlook_wnfg2}
    image_data_array, _ = convert_to_graph(image_data_array, methods['WNFG2'])

    labels_array = np.array(labels)
    # 每个标签重复 9 次
    labels_array = np.repeat(labels_array, 9)
    ...
    model = SSGCNnet(channels=image_data_array.shape[1], num_classes=num_classes).to(device)  # 将模型移动到 GPU
    ...
    optimizer = PruneAdam(model.named_parameters(), lr=1e-3, weight_decay=1e-3)
    ...
    for epoch in range(num_epochs):
        model.train()
        # 训练部分
        for inputs, targets in tqdm(train_loader, desc=f'PreTraining Epoch [{epoch + 1}/{num_epochs}]'):
            inputs, targets = inputs.to(device), targets.to(device)  # 将数据和标签移动到 GPU
            optimizer.zero_grad()
            output = model(inputs)
            loss = regularized_nll_loss(False, 5e-4, model, output, targets)
            loss.backward()
            optimizer.step()
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, targets in tqdm(train_loader, desc=f'Training Epoch [{epoch + 1}/{num_epochs}]'):
            ...
            loss = admm_loss(False, 5e-4, 1e-2, device, model, Z, U, outputs, targets)
            loss.backward()
            optimizer.step()
        ...
        X = update_X(model)
        Z = update_Z_l1(X, U, 5e-4, 1e-2) if False else update_Z(X, U, 0.99)
        U = update_U(U, X, Z)
        ...
        if test_accuracy > best_test_acc:
            ...
            # 立即保存最佳模型权重到文件
            torch.save(best_model_weights, './classification_checkpoint/SSGCNet/best_model_weights.pth')
    ...
    model = SSGCNnet(channels=image_data_array.shape[1], num_classes=num_classes).to(device)  # 将模型移动到 GPU
    # 训练完成后，使用最佳模型权重更新模型
    model.load_state_dict(torch.load('./classification_checkpoint/SSGCNet/best_model_weights.pth'))
    model.eval()
    ...
    # 在训练完成后调用此函数
    plot_and_save_loss_curves(train_losses, test_losses, filename='classification_checkpoint/SSGCNet/loss_curves.png')
