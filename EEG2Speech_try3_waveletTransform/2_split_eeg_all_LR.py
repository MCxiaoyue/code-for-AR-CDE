...
class EVRNet(nn.Module):
    def __init__(self, num_classes, input_size=24, hidden_size=256):
        super(EVRNet, self).__init__()

        # 线性回归层
        self.fc_layer_1 = nn.Linear(input_size, hidden_size)  # 输入维度为24，输出为1（类似于线性回归）
    ...
    def forward(self, x):
        ...
        # 应用线性回归层
        out = self.fc_layer_1(x.permute(0, 2, 1))  # 调整维度并应用线性回归
...
if __name__ == '__main__':
    ...
    for epoch in range(num_epochs):
        ...
        if test_accuracy > best_test_acc:
            ...
            # 立即保存最佳模型权重到文件
            torch.save(best_model_weights, './classification_checkpoint/LR/best_model_weights.pth')
    ...
    model = EVRNet(num_classes=num_classes).to(device)  # 将模型移动到 GPU
    # 训练完成后，使用最佳模型权重更新模型
    model.load_state_dict(torch.load('./classification_checkpoint/LR/best_model_weights.pth'))
    model.eval()
    ...
    # 在训练完成后调用此函数
    plot_and_save_loss_curves(train_losses, test_losses, filename='classification_checkpoint/LR/loss_curves.png')
