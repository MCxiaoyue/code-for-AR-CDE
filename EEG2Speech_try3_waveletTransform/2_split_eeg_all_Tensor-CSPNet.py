...
def _tensor_stack(x_fb):
        ...
        return stack_tensor
def get_filter_coeff(fs=128, f_pass=np.arange(4, 5, 4), f_width=36, f_trans=2, gpass=3, gstop=30): # f_pass=np.arange(4, 40, 4)
    filter_coeff = {}
    Nyquist_freq = fs / 2
...
def filter_data(eeg_data, window_details={'tmin': 0.0, 'tmax': 1.5}, fs=128):
...
if __name__ == '__main__':
    ...
    optimizer = optim.Adam(model.parameters(), lr=1e-2, weight_decay=1e-4)  # betas=(0.95, 0.999), eps=1e-08
    ...
    for epoch in range(num_epochs):
        ...
        if test_accuracy > best_test_acc:
            ...
            # 立即保存最佳模型权重到文件
            torch.save(best_model_weights, './classification_checkpoint/Tensor-CSPNet/best_model_weights.pth')
    ...
    model = Tensor_CSPNet_Basic(channel_num=image_data_array.shape[1]*image_data_array.shape[2], num_classes=num_classes).to(device)  # 将模型移动到 GPU
    # 训练完成后，使用最佳模型权重更新模型
    model.load_state_dict(torch.load('./classification_checkpoint/Tensor-CSPNet/best_model_weights.pth'))
    model.eval()
    ...
    # 在训练完成后调用此函数
    plot_and_save_loss_curves(train_losses, test_losses, filename='classification_checkpoint/Tensor-CSPNet/loss_curves.png')
