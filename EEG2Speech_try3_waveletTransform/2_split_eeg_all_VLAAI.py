...
class VLAAI(nn.Module):
    def __init__(self, nb_blocks=1, input_channels=24, num_classes=2, use_skip=True, extractor_output=256):
        ...
        self.final_dense = nn.Linear(input_channels, num_classes)  # 最终分类层
...
if __name__ == '__main__':
    ...
    model = VLAAI(num_classes=num_classes, input_channels=24).to(device)  # 将模型移动到 GPU
    ...
    for epoch in range(num_epochs):
        ...
        if test_accuracy > best_test_acc:
            ...
            # 立即保存最佳模型权重到文件
            torch.save(best_model_weights, './classification_checkpoint/VLAAI/best_model_weights.pth')
    ...
    model = VLAAI(num_classes=num_classes, input_channels=24).to(device)  # 将模型移动到 GPU
    # 训练完成后，使用最佳模型权重更新模型
    model.load_state_dict(torch.load('./classification_checkpoint/VLAAI/best_model_weights.pth'))
    model.eval()
    ...
    # 在训练完成后调用此函数
    plot_and_save_loss_curves(train_losses, test_losses, filename='classification_checkpoint/VLAAI/loss_curves.png')
