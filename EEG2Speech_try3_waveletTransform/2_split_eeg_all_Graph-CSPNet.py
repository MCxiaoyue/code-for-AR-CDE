...
def LGT_graph_matrix_fn(lattice, gamma=50, time_step=[2, 2, 2, 4], freq_step=[1, 1, 4, 3]): #  time_step=[1, 2, 3], freq_step=[1, 1, 1]
    ...
    '''
    time_freq_graph = {'1': [[0, 64], [64, 128], [128, 192]]}
    block_dims = [
...
if __name__ == '__main__':
    ...
    optimizer = optim.Adam(model.parameters(), lr=1e-3, weight_decay=1e-4)  # betas=(0.95, 0.999), eps=1e-08
    ...
    for epoch in range(num_epochs):
        ...
        if test_accuracy > best_test_acc:
            ...
            # 立即保存最佳模型权重到文件
            torch.save(best_model_weights, './classification_checkpoint/Graph-CSPNet/best_model_weights.pth')
    ...
    model = Graph_CSPNet_Basic(channel_num=image_data_array.shape[1], P=graph_M, num_classes=num_classes).to(device)  # 将模型移动到 GPU
    # 训练完成后，使用最佳模型权重更新模型
    model.load_state_dict(torch.load('./classification_checkpoint/Graph-CSPNet/best_model_weights.pth'))
    model.eval()
    ...
    # 在训练完成后调用此函数
    plot_and_save_loss_curves(train_losses, test_losses, filename='./classification_checkpoint/Graph-CSPNet/loss_curves.png')
