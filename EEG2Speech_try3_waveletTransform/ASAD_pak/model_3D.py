...
class DenseNet_3D(nn.Module):
    def __init__(self, num_classes):
        ...
        self.linear = nn.Linear(248, num_classes)
    ...
    def transition_block(self, in_channels, out_channels):
        blk = nn.Sequential(
            ...
            nn.AvgPool3d(kernel_size=(7, 1, 2), stride=(3, 1, 1))  # nn.AvgPool3d(kernel_size=(7, 2, 2), stride=(3, 1, 1))
        )
        return blk
...
if __name__ == '__main__':
    ...
    # 假设 batch_size=4, time_steps=100, n_electrodes=10, spatial_features=5
    x = torch.randn(64, 1, 192, 5, 7)  # 输入数据形状

    model = DenseNet_3D(num_classes=10)
