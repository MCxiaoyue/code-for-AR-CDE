...
class EVRNet(nn.Module):
    def __init__(self, num_classes):
        ...
        self.mamba = Mamba(
  num_layers = 5,    # Number of layers of the full model
  d_input = 24,     # Dimension of each vector in the input sequence (i.e. token size)
  ...
  parallel = False,  # Whether to use the sequenial or the parallel implementation
        ...
        self.fc_layer = nn.Linear(24, num_classes)  # 调整线性层的输入大小
    ...
    def forward(self, x):
        ...
        x = x.squeeze(1)  # Remove the unnecessary dimension, now shape is (batch_size, channels, timestamps)
        x = x.permute(0, 2, 1)  # Reorder dimensions to (batch_size, timestamps, channels)

        # x = x.view(x.size(0), x.size(3), -1)
        # print(x.shape)
        x = self.mamba(x)[0]

        # x = torch.mean(x, dim=1)  # Shape becomes (batch_size, hidden_size)
        ...
        # x = self.mamba(x)[0]
        # print(x.shape)
        # # x = x.view(x.size(0), x.size(1), 11, -1)
        # print(x.shape)
        ...
        # 在forward函数中
        x = x.permute(0, 2, 1)  # 调整为(batch_size, d_seq, seq_length)
        x = self.avg_pooling(x)  # 应用自适应平均池化
        x = x.squeeze(-1)  # 移除最后一个维度，变为(batch_size, d_seq)
        x = x.permute(0, 1)  # 如果需要的话，调整回原来的顺序
...
if __name__ == '__main__':
    ...
    for epoch in range(num_epochs):
        ...
        if test_accuracy > best_test_acc:
            ...
            # 立即保存最佳模型权重到文件
            torch.save(best_model_weights, './classification_checkpoint/Mamba/best_model_weights.pth')
    ...
    model = EVRNet(num_classes=num_classes).to(device)  # 将模型移动到 GPU
    # 训练完成后，使用最佳模型权重更新模型
    model.load_state_dict(torch.load('./classification_checkpoint/Mamba/best_model_weights.pth'))
    model.eval()
    ...
    # 在训练完成后调用此函数
    plot_and_save_loss_curves(train_losses, test_losses, filename='classification_checkpoint/Mamba/loss_curves.png')
