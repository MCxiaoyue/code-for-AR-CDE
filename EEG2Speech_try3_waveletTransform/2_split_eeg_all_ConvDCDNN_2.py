...
import math
from ConvDCDNN_pak.adcl import ADCL
from ConvDCDNN_pak.adcl1 import ImprovedADCL
from ConvDCDNN_pak.focal_loss import focal_loss
...
if __name__ == '__main__':
    ...
    # loss function
    criterion = focal_loss(alpha=None, gamma=2, device=device)  # alpha = [1.0] * num_classes
    # optimizer
    optimizer = ADCL(model.parameters(), lr=1e-3, weight_decay=1e-3)  # , weight_decay=1e-3
    # optimizer = ImprovedADCL(model.parameters(), lr=1e-4, weight_decay=1e-4)
    ...
    for epoch in range(num_epochs):
        ...
        if test_accuracy > best_test_acc:
            ...
            # 立即保存最佳模型权重到文件
            torch.save(best_model_weights, './classification_checkpoint/ConvDCDNN/best_model_weights.pth')
    ...
    model = EVRNet(num_classes=num_classes).to(device)  # 将模型移动到 GPU
    # 训练完成后，使用最佳模型权重更新模型
    model.load_state_dict(torch.load('./classification_checkpoint/ConvDCDNN/best_model_weights.pth'))
    model.eval()
    ...
    # 在训练完成后调用此函数
    plot_and_save_loss_curves(train_losses, test_losses, filename='classification_checkpoint/ConvDCDNN/loss_curves.png')
