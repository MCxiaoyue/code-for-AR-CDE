...
def sliding_window(eeg_datas, out_channels = 24, window_length = math.ceil(128 * 1.5), overlap = 0.5):
...
if __name__ == '__main__':
    ...
    gamma_low = 31
    gamma_high = 50
    window_length = math.ceil(128 * 1.5)
    frequency_resolution = 128 / window_length
    ...
    # 初始化 CSP
    csp = CSP(n_components=22, reg=None, log=None, cov_est='concat', transform_into='csp_space',
              norm_trace=True)
    # 使用 fit_transform 方法
    image_data_array = csp.fit_transform(image_data_array, labels_array)
    image_data_array = image_data_array.transpose(0, 2, 1)

    image_data_array = sliding_window(image_data_array, out_channels=22).transpose(0, 2, 1)
    ...
    model = DBPNet(in_channels=22, num_classes=num_classes).to(device)  # 将模型移动到 GPU
    ...
    for epoch in range(num_epochs):
        ...
        if test_accuracy > best_test_acc:
            ...
            # 立即保存最佳模型权重到文件
            torch.save(best_model_weights, './classification_checkpoint/DBPNet/best_model_weights.pth')
    ...
    model = DBPNet(in_channels=22, num_classes=num_classes).to(device).to(device)  # 将模型移动到 GPU
    # 训练完成后，使用最佳模型权重更新模型
    model.load_state_dict(torch.load('./classification_checkpoint/DBPNet/best_model_weights.pth'))
    model.eval()
    ...
    # 在训练完成后调用此函数
    plot_and_save_loss_curves(train_losses, test_losses, filename='classification_checkpoint/DBPNet/loss_curves.png')
