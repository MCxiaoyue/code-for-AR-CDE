...
from DARNet_pak.model import DARNet
...
if __name__ == '__main__':
    ...
    model = DARNet(num_classes=num_classes, data_shape=image_data_array[0].shape).to(device)  # 将模型移动到 GPU
    criterion = nn.CrossEntropyLoss().to(device)  # 将损失函数也移动到 GPU
    optimizer = optim.Adam(model.parameters(), lr=1e-5, weight_decay=1e-4)  # betas=(0.95, 0.999), eps=1e-08
    ...
    for epoch in range(num_epochs):
        ...
        if test_accuracy > best_test_acc:
            ...
            # 立即保存最佳模型权重到文件
            torch.save(best_model_weights, './classification_checkpoint/DARNet/best_model_weights.pth')
    ...
    model = DARNet(num_classes=num_classes, data_shape=image_data_array[0].shape).to(device)  # 将模型移动到 GPU
    # 训练完成后，使用最佳模型权重更新模型
    model.load_state_dict(torch.load('./classification_checkpoint/DARNet/best_model_weights.pth'))
    model.eval()
    ...
    # 在训练完成后调用此函数
    plot_and_save_loss_curves(train_losses, test_losses, filename='classification_checkpoint/DARNet/loss_curves.png')
