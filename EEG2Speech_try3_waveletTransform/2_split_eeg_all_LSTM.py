...
class EVRNet(nn.Module):
    def __init__(self, num_classes, lstm_hidden_size=256, num_lstm_layers=1):
        ...
        self.lstm = nn.LSTM(input_size=24, hidden_size=self.lstm_hidden_size, num_layers=self.num_lstm_layers,
                            batch_first=True)

        self.avg_pooling = nn.AdaptiveAvgPool1d(1)  # 自适应平均池化层
        # Fully connected layer
        self.fc_layer = nn.Linear(self.lstm_hidden_size, num_classes)
    ...
    def forward(self, x):
        ...
        h_0 = torch.zeros(self.num_lstm_layers, x.size(0), self.lstm_hidden_size).to(x.device)
        c_0 = torch.zeros(self.num_lstm_layers, x.size(0), self.lstm_hidden_size).to(x.device)

        out, _ = self.lstm(x, (h_0, c_0))  # out: tensor of shape (batch_size, seq_length, hidden_size)

        # Average pooling over all time steps
        # out = torch.mean(out, dim=1)  # Shape becomes (batch_size, hidden_size)
        # out = self.avg_pooling(out)
        # out = out.view(out.size(0), -1)

        # 在forward函数中
        out = out.permute(0, 2, 1)  # 调整为(batch_size, d_seq, seq_length)
        out = self.avg_pooling(out)  # 应用自适应平均池化
        out = out.squeeze(-1)  # 移除最后一个维度，变为(batch_size, d_seq)
        out = out.permute(0, 1)  # 如果需要的话，调整回原来的顺序
...
if __name__ == '__main__':
    ...
    for epoch in range(num_epochs):
        ...
        if test_accuracy > best_test_acc:
            ...
            # 立即保存最佳模型权重到文件
            torch.save(best_model_weights, './classification_checkpoint/LSTM/best_model_weights.pth')
    ...
    model = EVRNet(num_classes=num_classes).to(device)  # 将模型移动到 GPU
    # 训练完成后，使用最佳模型权重更新模型
    model.load_state_dict(torch.load('./classification_checkpoint/LSTM/best_model_weights.pth'))
    model.eval()
    ...
    # 在训练完成后调用此函数
    plot_and_save_loss_curves(train_losses, test_losses, filename='classification_checkpoint/LSTM/loss_curves.png')
