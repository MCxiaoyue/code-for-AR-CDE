...
class NeuroInceptDecoder(nn.Module):
    def __init__(self, input_shape, num_classes):
        ...
        # Inception Module 2
        self.inception2 = InceptionModule(192, 128)
...
def process_file(file_path, image_data, labels):
    with open("./orign_eeg_data/"+file_path, mode='r') as f:
        ...
        for eeg in word_data_segments:
            # print(eeg.shape)
            # 对每个通道进行归一化
            scaler = StandardScaler()  # RobustScaler()  # MinMaxScaler() RobustScaler() StandardScaler()
...
if __name__ == '__main__':
    ...
    for epoch in range(num_epochs):
        ...
        if test_accuracy > best_test_acc:
            ...
            # 立即保存最佳模型权重到文件
            torch.save(best_model_weights, './classification_checkpoint/NeuroIncept/best_model_weights.pth')
    ...
    model = NeuroInceptDecoder(input_shape=np.array(image_data_array[0]).shape, num_classes=num_classes).to(device)  # 将模型移动到 GPU
    # 训练完成后，使用最佳模型权重更新模型
    model.load_state_dict(torch.load('./classification_checkpoint/NeuroIncept/best_model_weights.pth'))
    model.eval()
    ...
    # 在训练完成后调用此函数
    plot_and_save_loss_curves(train_losses, test_losses, filename='classification_checkpoint/NeuroIncept/loss_curves.png')
