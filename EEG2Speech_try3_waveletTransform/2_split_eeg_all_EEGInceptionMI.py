...
from braindecode.models import EEGNetv4, ATCNet, EEGConformer, EEGITNet, ShallowFBCSPNet, EEGInception, EEGInceptionMI
import torch.nn as nn
...
class CustomEEGInceptionMI(nn.Module):
    def __init__(self, num_classes, in_chans=24, input_window_samples=192):
        super(CustomEEGInceptionMI, self).__init__()

        # 初始化 ATCNet 模型，根据需要调整参数
        self.atcnet = EEGInceptionMI(
            n_chans=in_chans,
            ...
            sfreq=128,
            n_filters=16,
            kernel_unit_s=0.0546875,
        )

    def forward(self, x):
        # 确保输入张量的维度符合 EEGInception 的期望 [batch_size, channels, time_points]
        if x.dim() == 4:  # 如果输入是4D张量，移除不必要的维度
            x = x.squeeze(1)  # 假设多余的维度在第1个位置
        # print(x.shape)
        # 通过 EEGInception 模型进行前向传播
        x = self.atcnet(x)
        # print(x.shape)
...
if __name__ == '__main__':
    ...
    file_paths = ['vDLY-001_1.txt', 'vLCL-001_1.txt', 'vLHJ-001_1.txt', 'vLHJ-002_1.txt', 'vLCL-004_1.txt',
                  'vYHC02_1.txt', 'vLCL-01_1.txt', 'vLCL-02_1.txt', 'vCLB-001_1.txt', 'vYHC-001_1.txt',
                  'vLCL-003_1.txt']
    ...
    model = CustomEEGInceptionMI(num_classes=10, in_chans=24).to(device)  # EVRNet(num_classes=num_classes).to(device)  # 将模型移动到 GPU
    ...
    for epoch in range(num_epochs):
        ...
        if test_accuracy > best_test_acc:
            ...
            # 立即保存最佳模型权重到文件
            torch.save(best_model_weights, './classification_checkpoint/EEGInceptionMI/best_model_weights.pth')
    ...
    model = CustomEEGInceptionMI(num_classes=10, in_chans=24).to(device)  # 将模型移动到 GPU
    # 训练完成后，使用最佳模型权重更新模型
    model.load_state_dict(torch.load('./classification_checkpoint/EEGInceptionMI/best_model_weights.pth'))
    model.eval()
    ...
    for i in range(len(all_predicted_labels)):
        ...
        # 在训练完成后调用此函数
    plot_and_save_loss_curves(train_losses, test_losses,
                              filename='classification_checkpoint/EEGInceptionMI/loss_curves.png')