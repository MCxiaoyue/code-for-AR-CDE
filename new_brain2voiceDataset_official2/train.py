...
from net import UNITGenerator1
...
## 训练判别器A ##
def D_A_train(D_A: Discriminator1, G_B: UNITGenerator1, X, Y, BCELoss, optimizer_D):
    # print(X.shape)
    # print(Y.shape)
    # print('==========================================')
    """
...
## 训练判别器B ##
def D_B_train(D_B: Discriminator1, G_A: UNITGenerator1, X, Y, BCELoss, optimizer_D):
    """
    训练判别器B
    :param BCELoss: 二分交叉熵损失函数
    :param optimizer_D: 判别器优化器
...
## 训练生成器 ##
def G_train(D_A: Discriminator1, D_B: Discriminator1, G_A: UNITGenerator1, G_B: UNITGenerator1, X, Y, BCELoss, L1, optimizer_G,
            lamb=100):
    """
    训练生成器
    :param BCELoss: 二分交叉熵损失函数
...
def main():
    ...
    # 声明生成器、判别器
    G_A = UNITGenerator1.Generator(in_ch, out_ch, ngf).to(device)  # 生成B
    G_B = UNITGenerator1.Generator(in_ch, out_ch, ngf).to(device)  # 生成A
