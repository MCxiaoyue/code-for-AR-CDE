...
from tqdm import tqdm
from braindecode.models import EEGNetv1, ATCNet, EEGConformer, EEGITNet, ShallowFBCSPNet, EEGConformer
import random
...
class CustomATCNet(nn.Module):
    def __init__(self, num_classes, in_chans=124, input_window_samples=512):
        super(CustomATCNet, self).__init__()

        # 定义 EEGNetv4 模型，调整 input_window_samples 以匹配您的数据
        self.eegnet = ATCNet(
            n_chans=in_chans,
            input_window_seconds= 0.5,# 2.0009765625,
            sfreq=1024,
...
tag = 's'
duration = 2
subjects = ['15']  # 添加所有需要处理的主题ID  , '12', '11', '17'
...
for subject in subjects:
    for session in sessions:
        ...
        # imagine_path = f'E:\\日常文献查询\\相关代码\\Semantics-EEG-Perception-and-Imagination-main_dataset\\derivatives\\preprocessed\\epochs\\imagine_{task}\\'
...
for channel in range(n_channels):
    for sample in range(n_samples):
        ...
        f = interp1d(x_original, current_data, kind='cubic')  # 创建插值函数
...
#         count += 1
#         if count == 1:
#             ica_data = np.zeros((n_samples, n_channels, data.shape[1]))
...
# 初始化模型、损失函数和优化器
num_classes = 3
model = CustomATCNet(num_classes=num_classes, in_chans=124, input_window_samples=512).to(device)  # 将模型移动到 GPU
...
model = CustomATCNet(num_classes=num_classes, in_chans=124, input_window_samples=2049).to(device) # 将模型移动到 GPU
# 训练完成后，使用最佳模型权重更新模型
