...
class Extractor(nn.Module):
    def __init__(self, filters=(256, 256, 256, 256, 256), kernels=(8,) * 5, input_channels=24):  # 修改 input_channels=24
...
class OutputContext(nn.Module):
    def __init__(self, filter_=24, kernel=32, input_channels=24):
...
class VLAAI(nn.Module):
    def __init__(self, nb_blocks=1, input_channels=124, num_classes=3, use_skip=True, extractor_output=256):
        super(VLAAI, self).__init__()
        self.nb_blocks = nb_blocks
        self.use_skip = use_skip
        self.extractor = Extractor(input_channels=input_channels)  # 设置 input_channels=24
        self.dense = nn.Linear(extractor_output, input_channels)  # Dense 层连接到 input_channels
        self.output_context = OutputContext(input_channels=input_channels)  # 设置 input_channels=24
        self.final_dense = nn.Linear(24, num_classes)  # 最终分类层

    def forward(self, x):
        x = x.squeeze(1)
        """
        输入: (batch_size, channels, timestamps)
        输出: (batch_size, num_classes)
        """
        for _ in range(self.nb_blocks):
            skip = x if self.use_skip else 0
            x = self.extractor(x + skip)
            x = x.transpose(1, 2)  # (batch_size, timestamps, channels)
            x = self.dense(x)  # Dense layer over channels
            x = x.transpose(1, 2)  # Back to (batch_size, channels, timestamps)
            x = self.output_context(x)

        # 全局平均池化，将时间维度压缩为1
        x = torch.mean(x, dim=-1)  # (batch_size, channels)

        # 最终全连接层进行分类
        x = self.final_dense(x)  # (batch_size, num_classes)
...
# 初始化模型、损失函数和优化器
num_classes = 3
model = VLAAI(num_classes=num_classes, input_channels=124).to(device)  # 将模型移动到 GPU
...
for epoch in range(num_epochs):
    ...
    if test_accuracy > best_test_acc:
        ...
        torch.save(best_model_weights,
                   'classification_checkpoint/' + str(subjects[0]) + '/VLAAI/best_model_weights.pth')
...
model = VLAAI(num_classes=num_classes, input_channels=124).to(device)  # 将模型移动到 GPU
# 训练完成后，使用最佳模型权重更新模型
model.load_state_dict(torch.load('./classification_checkpoint/' + str(subjects[0]) + '/VLAAI/best_model_weights.pth'))
model.eval()
...
# 在训练完成后调用此函数
plot_and_save_loss_curves(train_losses, test_losses, filename='classification_checkpoint/' + str(subjects[0]) + '/VLAAI/loss_curves.png')
