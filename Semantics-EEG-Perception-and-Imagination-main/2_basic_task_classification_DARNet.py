...
for epoch in range(num_epochs):
    ...
    if test_accuracy > best_test_acc:
        ...
        torch.save(best_model_weights,
                   'classification_checkpoint/' + str(subjects[0]) + '/DARNet/best_model_weights.pth')
...
model = DARNet(num_classes=num_classes, data_shape=image_data_array[0].shape).to(device)  # 将模型移动到 GPU
# 训练完成后，使用最佳模型权重更新模型
model.load_state_dict(torch.load('./classification_checkpoint/' + str(subjects[0]) + '/DARNet/best_model_weights.pth'))
model.eval()
...
# 在训练完成后调用此函数
plot_and_save_loss_curves(train_losses, test_losses, filename='classification_checkpoint/' + str(subjects[0]) + '/DARNet/loss_curves.png')
