...
# 取前 2048 个采样点
image_data_array = image_data_array[:, :, :2048]
# 定义目标采样率和原始采样率
...
for sample in image_data_array:
    ...
    # 将展平后的数组按每 1028 个数据点分割
    split_parts = np.array_split(flat_sample, 31)
    # 确保每部分恰好有 1028 个数据点
    for part in split_parts:
        if len(part) == 1024:  # 验证长度是否正确
            reshaped_data.append(part)
# 将列表转换为 NumPy 数组并调整形状为 (-1, 1028)
image_data_array = np.array(reshaped_data).reshape(-1, 1024)
...
# 每个标签重复 31 次
labels_array = np.repeat(labels_array, 31)
