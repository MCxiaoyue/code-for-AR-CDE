...
from ASAD_pak.model_2D import DenseNet_2D
...
# 初始化模型、损失函数和优化器
num_classes = 3
model = DenseNet_2D(num_classes=num_classes).to(device)  # 将模型移动到 GPU
...
for epoch in range(num_epochs):
    ...
    if test_accuracy > best_test_acc:
        ...
        torch.save(best_model_weights,
                   'classification_checkpoint/' + str(subjects[0]) + '/ASAD_2D/best_model_weights.pth')
...
model = DenseNet_2D(num_classes=num_classes).to(device)  # 将模型移动到 GPU
# 训练完成后，使用最佳模型权重更新模型
model.load_state_dict(torch.load('./classification_checkpoint/' + str(subjects[0]) + '/ASAD_2D/best_model_weights.pth'))
model.eval()
...
# 在训练完成后调用此函数
plot_and_save_loss_curves(train_losses, test_losses, filename='classification_checkpoint/' + str(subjects[0]) + '/ASAD_2D/loss_curves.png')
