...
from tqdm import tqdm
from braindecode.models import EEGNetv1, ATCNet, EEGConformer, EEGITNet, ShallowFBCSPNet
import random
...
class CustomEEGNetv1(nn.Module):
    def __init__(self, num_classes, in_chans=124, input_window_samples=2049, final_conv_length='auto'):
...
# 初始化模型、损失函数和优化器
num_classes = 3
model = CustomEEGNetv1(num_classes=num_classes, in_chans=124, input_window_samples=2049).to(device)  # 将模型移动到 GPU
criterion = nn.CrossEntropyLoss().to(device)  # 将损失函数也移动到 GPU
optimizer = optim.Adam(model.parameters(), lr=1e-4, weight_decay=5e-4)  # betas=(0.95, 0.999), eps=1e-08
