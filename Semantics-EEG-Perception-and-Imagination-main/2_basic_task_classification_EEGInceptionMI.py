...
from tqdm import tqdm
from braindecode.models import EEGNetv1, ATCNet, EEGConformer, EEGITNet, ShallowFBCSPNet, EEGConformer, EEGInception, EEGInceptionERP, EEGInceptionMI
import random
...
tag = 's'
duration = 2
subjects = ['8']  # 添加所有需要处理的主题ID  , '12', '11', '17'
...
# 初始化模型、损失函数和优化器
num_classes = 3
model = CustomEEGInceptionMI(num_classes=num_classes, in_chans=124, input_window_samples=2049).to(device)  # 将模型移动到 GPU
