...
class Tensor_CSPNet_Basic(nn.Module):
    ...
    def __init__(self, channel_num, num_classes, mlp = False):
        ...
        self.tcn_width = 1
        self.Temporal_Block = nn.Conv2d(1, self.tcn_channels, (self.kernel_size, self.tcn_width * self.dims[-1] ** 2),
                                        stride=(1, self.dims[-1] ** 2), padding=0).double()
    ...
    def _make_BiMap_block(self, layer_num):
        ...
        dim_in, dim_out = self.dims[-1], self.dims[-1]
        layers.append(BiMap(self.channel_in, dim_in, dim_out))
...
class Graph_CSPNet_Basic(nn.Module):
    ...
    def __init__(self, channel_num, P, num_classes, mlp = False):
