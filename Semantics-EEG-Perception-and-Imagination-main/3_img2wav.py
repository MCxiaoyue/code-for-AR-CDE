...
n_mels = 200  # 设置生成 Mel 滤波器组的数量为 80。Mel 频率倒谱系数（MFCC）是一种常用的音频特征提取方法，这里设置了用于提取 MFCC 的 Mel 滤波器组数量。
...
# file_names = ["SA1_", "SA1_1", "SA1_2"]

file_names = [i for i in range(6)]

# file_names = [i for i in range(1, 27, 5)]
# flag = [0, 1, 2, 0, 1, 2]
index = 0

for name in file_names:
    # input_image = Image.open("./generated_images/generated_"+str(name)+"_torchvision.png")
    input_image = Image.open("./results/comparison_results/Recon_" + str(name) + "___.png")

    # input_image = Image.open("./3words2/B_" + str(name) + "_" + str(flag[index]) + ".png")
    ...
    output_image.save("./results/comparison_results/Recon_"+str(index)+"_.png")

    # 加载图像文件并转换为数组
    mel_img = mpimg.imread("./results/comparison_results/Recon_"+str(index)+"_.png")
    ...
    # 最后，将音频波形写入到.wav文件
    sf.write("./results/comparison_results/" + str(index) + "_.wav", wav1, sr)
    index += 1
