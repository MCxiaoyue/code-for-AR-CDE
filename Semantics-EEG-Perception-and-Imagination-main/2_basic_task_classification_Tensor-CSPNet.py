...
from TensorCSPNet_pak.model import Tensor_CSPNet_Basic
...
def _tensor_stack(x_fb):
        ...
        # 将 stack_tensor 转换为 PyTorch 张量并设置为 double 类型
        stack_tensor = torch.from_numpy(stack_tensor).double()
        return stack_tensor

# def _tensor_stack(x_fb):
#     # 标准化数据
#     scaler = StandardScaler()
#     x_fb = scaler.fit_transform(x_fb.reshape(-1, x_fb.shape[-1])).reshape(x_fb.shape)
#
#     time_seg = [[0, 85], [85, 170], [170, 257]]
#     temporal_seg = []
#     for [a, b] in time_seg:
#         segment = np.expand_dims(x_fb[:, :, :, a:b], axis=1)
#         # 替换NaN和Inf值为0
#         segment = np.nan_to_num(segment, nan=0.0, posinf=0.0, neginf=0.0)
#         temporal_seg.append(segment)
#     temporal_seg = np.concatenate(temporal_seg, axis=1)
#
#     stack_tensor = []
#     for i in range(temporal_seg.shape[0]):
...
#         for j in range(temporal_seg.shape[1]):
#             # 计算协方差矩阵前再次标准化数据
#             temp_data = temporal_seg[i, j]
#             temp_data = scaler.fit_transform(temp_data.reshape(-1, temp_data.shape[-1])).reshape(temp_data.shape)
#
#             try:
#                 cov_matrix = Covariances().transform(temp_data)
#                 if np.any(np.isnan(cov_matrix)) or np.any(np.isinf(cov_matrix)):
#                     # print(f"Covariance calculation resulted in NaN/Inf for segment {i}, {j}")
#                     cov_matrix = np.nan_to_num(cov_matrix, nan=0.0, posinf=0.0, neginf=0.0)
#
#                 shrunk_cov_matrix = Shrinkage(1e-2).transform(cov_matrix)
#                 cov_stack.append(shrunk_cov_matrix)
#             except Exception as e:
#                 # print(f"Error processing segment {i}, {j}: {e}")
#                 cov_stack.append(np.zeros_like(cov_matrix))  # 或者使用其他合理的默认值
#
#         stack_tensor.append(np.stack(cov_stack, axis=0))
#     stack_tensor = np.stack(stack_tensor, axis=0)
#
...
def get_filter_coeff(fs=1024, f_pass=np.arange(4, 5, 4), f_width=36, f_trans=2, gpass=3,
                     gstop=30):  # f_pass=np.arange(4, 40, 4)
    filter_coeff = {}
    Nyquist_freq = fs / 2
...
subjects = ['15', '22']  # '12', '20'   '14', '21'   '15', '22'
...
print(filter.shape)
image_data_array = _tensor_stack(filter)
...
# 初始化模型、损失函数和优化器
num_classes = 3
model = Tensor_CSPNet_Basic(channel_num=image_data_array.shape[1]*image_data_array.shape[2], num_classes=num_classes).to(device)  # 将模型移动到 GPU
...
for epoch in range(num_epochs):
    ...
    if test_accuracy > best_test_acc:
        ...
        torch.save(best_model_weights,
                   'classification_checkpoint/' + str(subjects[0]) + '/Tensor-CSPNet/best_model_weights.pth')
...
model = Tensor_CSPNet_Basic(channel_num=image_data_array.shape[1]*image_data_array.shape[2], num_classes=num_classes).to(device)  # 将模型移动到 GPU
# 训练完成后，使用最佳模型权重更新模型
model.load_state_dict(torch.load('./classification_checkpoint/' + str(subjects[0]) + '/Tensor-CSPNet/best_model_weights.pth'))
model.eval()
...
# 在训练完成后调用此函数
plot_and_save_loss_curves(train_losses, test_losses, filename='classification_checkpoint/' + str(subjects[0]) + '/Tensor-CSPNet/loss_curves.png')
